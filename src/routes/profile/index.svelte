...
<script lang="ts">
    ...
    async function addLink(){
        const res = await fetch(`/publish/${link.replace(/^https?:\/\//,'')}`)
...
<div class="mt-10 max-w-screen-md mx-auto space-y-10">
    {#each profile.links as {data:link}, index}
            <a href="{link.url}" class="flex">
                ...
                <div>
                    ...
                    <p class="text-lg mt-2 line-clamp-4">{link.description}</p>
                </div>
            </a>
