...
export async function handle({ request, resolve }: { request: Request, resolve: (request: Request) => Response | Promise<Response> }) {
    ...
    const { user } = await auth.api.getUserByCookie(expressifyReq(request))
    request.locals.user = user || RESP_USER_GUEST // Converts request to have `req.headers.cookie` on `req.cookies, as `getUserByCookie` expects parsed cookies on `req.cookies`
    // prepare response
